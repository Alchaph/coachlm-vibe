...
test('custom prompt field appears between AI Model and Strava sections', async ({ page }) => {
  // Wait for all sections to render before reading text
  await expect(page.locator('section h2', { hasText: 'AI Model' })).toBeVisible()
  await expect(page.locator('section h2', { hasText: 'Custom Instructions' })).toBeVisible()
  await expect(page.locator('section h2', { hasText: 'Strava Connection' })).toBeVisible()

  const sections = page.locator('section h2')
  const texts = await sections.allTextContents()
  const aiIdx = texts.findIndex(t => t.includes('AI Model'))
  ...
  expect(customIdx).toBeGreaterThan(aiIdx)
